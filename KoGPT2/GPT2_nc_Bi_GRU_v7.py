...
# %%
train = pd.read_csv('./doc_v9_train.txt')
test = pd.read_csv('./doc_v9_test.txt')
# %%
...
for i in tqdm(range(len(train['sentence']))):
    ...
    sentence_ = sentence_.replace('[', '').replace(
        ']', '').replace("'", '').replace(",", '')
    lst.append(sentence_)
train['sentence'] = lst
# %%
...
# %%
history = model.fit(train_p, train_dummy_y, batch_size=512,
                    epochs=100, validation_data=(test_p, test_dummy_y))
